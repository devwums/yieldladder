...
mod test {
    ...
    fn test_deposit_records_balance_and_lock() {
        ...
        assert_eq!(client.balance(&user, &usdc), 1_000_000_000i128);
    ...
    fn test_positions_are_scoped_per_asset() {
        ...
        client.deposit(&user, &usdc, &1_000_000_000i128);
        client.deposit(&user, &eurc, &1_200_000_000i128);

        assert_eq!(client.balance(&user, &usdc), 1_000_000_000i128);
        assert_eq!(client.balance(&user, &eurc), 1_200_000_000i128);
    }
    ...
    fn test_withdraw_after_maturity_succeeds() {
        ...
        let payout = client.withdraw(&user, &usdc, &1_000_000_000i128);

        assert_eq!(payout, 1_000_000_000i128);
    }
    ...
    fn test_early_exit_applies_fee() {
        ...
        let net = client.early_exit(&user, &usdc, &1_000_000_000i128);

        // 1.25% fee on 1_000_000_000 = 12_500_000
        assert_eq!(net, 987_500_000i128);
    }
